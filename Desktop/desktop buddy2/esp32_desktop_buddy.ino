...
// mouth geometry
const int mouthCX = SCREEN_WIDTH / 2;
const int mouthCY = 52;
...
// sign = 1 -> smile (bowl, corners up), sign = -1 -> frown (dome, corners down)
void drawCurveMouth(int cx, int cy, int halfWidth, int amplitude, int thickness, int sign) {
  const int steps = 12;
  int prevX = 0, prevY = 0;
  for (int i = 0; i <= steps; i++) {
    float x = -halfWidth + (2.0 * halfWidth * i) / steps;
    float norm = x / (float)halfWidth;
    int y = cy + sign * (int)(amplitude * (1 - norm * norm));
    int px = cx + (int)x;
    if (i > 0) drawThickLine(prevX, prevY, px, y, thickness);
    prevX = px; prevY = y;
  }
}
...
void renderNormal(unsigned long now) {
  ...
  // small neutral mouth, drifts with the same breathing motion
  drawThickLine(mouthCX - 10, mouthCY + by, mouthCX + 10, mouthCY + by, 2);
}
...
void renderHappy(unsigned long now) {
  ...
  drawCurveMouth(mouthCX, mouthCY, 22, 8, 3, 1); // big open smile
}
...
void renderSleepy(unsigned long now) {
  ...
  // slow pulsing yawn
  int yawnR = 3 + (int)(sin(now / 1500.0) * 2);
  display.fillCircle(mouthCX, mouthCY + 4, yawnR, SSD1306_WHITE);
  display.fillCircle(mouthCX, mouthCY + 4, max(1, yawnR - 2), SSD1306_BLACK);
}
...
void renderSurprised(unsigned long now) {
  ...
  display.fillCircle(mouthCX, mouthCY, 7, SSD1306_WHITE);
  display.fillCircle(mouthCX, mouthCY, 4, SSD1306_BLACK);
}
...
void renderAngry(unsigned long now) {
  ...
  // flat, gritted-teeth mouth
  int mw = 18;
  drawThickLine(mouthCX - mw, mouthCY, mouthCX + mw, mouthCY, 3);
  for (int tx = -mw + 4; tx <= mw - 4; tx += 6) {
    display.drawLine(mouthCX + tx, mouthCY, mouthCX + tx, mouthCY + 5, SSD1306_WHITE);
  }
}
...
void renderWink(unsigned long now) {
  ...
  // playful smirk
  drawThickLine(mouthCX - 12, mouthCY + 2, mouthCX + 10, mouthCY - 4, 3);
}
...
void renderLove(unsigned long now) {
  ...
  drawCurveMouth(mouthCX, mouthCY, 15, 5, 3, 1);
}
...
void renderDizzy(unsigned long now) {
  ...
  // wavy, confused mouth
  int mw = 16;
  int prevX = mouthCX - mw, prevY = mouthCY;
  for (int i = 1; i <= 4; i++) {
    int x = mouthCX - mw + (2 * mw * i) / 4;
    int y = mouthCY + ((i % 2 == 0) ? 4 : -4);
    drawThickLine(prevX, prevY, x, y, 2);
    prevX = x; prevY = y;
  }
}
...
void renderSuspicious(unsigned long now) {
  ...
  // flat smirk that drifts with the eyes
  drawThickLine(mouthCX - 14 + shift, mouthCY, mouthCX + 6 + shift, mouthCY, 2);
}
